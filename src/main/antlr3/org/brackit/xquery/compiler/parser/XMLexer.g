...
LAngle       : { !isInTag() }? => '<'  ;
RAngle       : {  isInTag() }? => '>'  { unsetInTag(); };
LClose       : { !isInTag() }? => '</' { setInTag();  };
RClose       : {  isInTag() }? => '/>' { unsetInTag(); };
SymEq        : {  isInTag() }? => '=';
LCurly       : { !isInTag() || isInAposAttr() || isInQuotAttr() }? => '{';
RCurly       : { !isInTag() || isInAposAttr() || isInQuotAttr() }? => '}';
Quot         : {  isInTag() || isInAposAttr() }? => '"'  { negateInQuotAttr(); };
Apos         : {  isInTag() || isInQuotAttr() }? => '\'' { negateInAposAttr(); };
EscapeQuot   : { isInQuotAttr() }? => '""';
EscapeApos   : { isInAposAttr() }? => '\'\'';
EscapeLCurly : { !isInTag() || isInAposAttr() || isInQuotAttr() }? => '{{';
EscapeRCurly : { !isInTag() || isInAposAttr() || isInQuotAttr() }? => '}}';
Colon        : ':';
...
AposAttrContentChar  // Char - ['{}<&]
    ...
    ;
PredefinedEntityRef
    : { !isInTag() || isInAposAttr() || isInQuotAttr() }? =>
      '&' ('lt' | 'gt' | 'apos' | 'quot' | 'amp' ) ';'
    ;
CharRef
    : { !isInTag() || isInAposAttr() || isInQuotAttr() }? =>
